...
BEGIN
    ...
    IF p_brands IS NOT NULL AND array_length(p_brands, 1) > 0 THEN
        v_where := v_where || ' AND (' ||
            array_to_string(ARRAY(SELECT format('"brand" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_brands) s), ' OR ')
            || ') ';
    END IF;
    IF p_not_brands IS NOT NULL AND array_length(p_not_brands, 1) > 0 THEN
        v_where := v_where || ' AND NOT (' ||
            array_to_string(ARRAY(SELECT format('"brand" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_not_brands) s), ' OR ')
            || ') ';
    END IF;
     IF p_ic1 IS NOT NULL AND array_length(p_ic1, 1) > 0 THEN
        v_where := v_where || ' AND (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass1" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_ic1) s),
                ' OR '
            ) || ') ';
    END IF;
    IF p_not_ic1 IS NOT NULL AND array_length(p_not_ic1, 1) > 0 THEN
        v_where := v_where || ' AND NOT (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass1" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_not_ic1) s),
                ' OR '
            ) || ') ';
    END IF;
    --------------------------------------------------------
    ...
    IF p_ic2 IS NOT NULL AND array_length(p_ic2, 1) > 0 THEN
        v_where := v_where || ' AND (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass2" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_ic2) s),
                ' OR '
            ) || ') ';
    END IF;
    IF p_not_ic2 IS NOT NULL AND array_length(p_not_ic2, 1) > 0 THEN
        v_where := v_where || ' AND NOT (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass2" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_not_ic2) s),
                ' OR '
            ) || ') ';
    END IF;
    --------------------------------------------------------
    ...
    IF p_ic3 IS NOT NULL AND array_length(p_ic3, 1) > 0 THEN
        v_where := v_where || ' AND (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass3" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_ic3) s),
                ' OR '
            ) || ') ';
    END IF;
    IF p_not_ic3 IS NOT NULL AND array_length(p_not_ic3, 1) > 0 THEN
        v_where := v_where || ' AND NOT (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass3" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_not_ic3) s),
                ' OR '
            ) || ') ';
    END IF;
    --------------------------------------------------------
    ...
    IF p_ic4 IS NOT NULL AND array_length(p_ic4, 1) > 0 THEN
        v_where := v_where || ' AND (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass4" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_ic4) s),
                ' OR '
            ) || ') ';
    END IF;
    IF p_not_ic4 IS NOT NULL AND array_length(p_not_ic4, 1) > 0 THEN
        v_where := v_where || ' AND NOT (' ||
            array_to_string(
                ARRAY(SELECT format('"itemClass4" ILIKE %L', CASE WHEN position('%' in s) > 0 THEN s ELSE s || '%' END) FROM unnest(p_not_ic4) s),
                ' OR '
            ) || ') ';
    END IF;
      v_sql := '
