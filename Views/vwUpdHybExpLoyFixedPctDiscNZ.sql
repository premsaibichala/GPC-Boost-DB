        ...
        UNION ALL
         SELECT eo."eventId" AS event_id,
           ...
           FROM "tEvent" ev
             ...
             JOIN "tConfig" tc ON ev.country::text = tc.configkey::text AND tc.configtype::text = 'COUNTRY'::text
             LEFT JOIN "tHybrisStickerText" hst ON eo."hybrisStickerText"::text = hst."hybrisStickerText"::text AND ev.country::text = hst.country::text
          WHERE ev.country::text = 'NZ'::text AND ev.locked = true AND eo."isNotAvailableOnline" = false AND eod."advertisedPrice" > 0::numeric AND ot."offerTypeId" IN (14, 114) AND eo."isRewards" = true AND NOT (ev."eventType"::text = 'Retail Catalogue'::text AND eo."pagePosition" = 0)
